  ...
  const fetchOrders = async (userId: string) => {
    try {
      const ordersResponse = await axios.get(
        `/api/Order/get-user-orders/${userId}`
      );
      ...
      const orderDetails = await Promise.all(
        ordersData.map(async (order: Order) => {
          const ticketResponse = await axios.get(
            `/api/Ticket/get-ticket/${order.ticketId}`
          );
          const ticketsData: Ticket[] = Array.isArray(ticketResponse.data)
            ? ticketResponse.data
            : [ticketResponse.data];
          ...
          for (const ticket of ticketsData) {
            ...
            if (!uniqueSellerIds.has(ticket.userId)) {
              const sellerResponse = await axios.get(
                `/api/Account/user-information/${ticket.userId}`
              );
          ...
          const seller =
            uniqueSellerIds.size > 0
              ? Array.from(uniqueSellerIds.values())[0]
              : {
                  id: "unknown",
                  firstName: "Unknown",
                  lastName: "Seller",
